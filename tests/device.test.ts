...
import type consoleTests from '#/fixtures/Tests/Parser/Device/fixtures/console.json';
...
const deviceTester = (tests: typeof cameraTests | typeof carTests | typeof consoleTests) => {
  for (const unitTest of tests) {
    ...
    test(`${brand} ${unitTest.device.model || ''}`, () => {
      ...
      unitTest.device.type = unitTest.device.type.replace('car browser', 'car').replace('tv', 'television');
